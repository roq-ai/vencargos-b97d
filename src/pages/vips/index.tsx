...
import { getVips, deleteVipById } from 'apiSdk/vips';
import { VipInterface } from 'interfaces/vip';

type ColumnType = ColumnDef<VipInterface, unknown>;
...
export function VipListPage(props: VipListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getVips({
        relations: ['user', 'content_creator.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<VipInterface>>(
    () => `/vips?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: VipInterface) => {
    if (hasAccess('vip', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/vips/view/${row.id}`);
    }
  };
...

    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/vips/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('vip', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/vips/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('vip', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="vip" />
        </Flex>
        {hasAccess('vip', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/vips/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
