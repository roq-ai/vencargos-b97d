...
import { getVips } from 'apiSdk/vips';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['agencies', 'content-creator'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<VipInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/vips` : null),
    () => getVips({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Vips',
      path: isTenantUser ? `/vips/view/${data?.[0]?.id}` : '/vips',
      entity: 'vip',
